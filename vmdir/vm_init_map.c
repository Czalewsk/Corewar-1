...
static void	vm_init_proc(t_vm_data *data, t_vm_champ *champ, int pos)
{
    if (!data->tab_proc)
    {
        ...
        data->nb_proc = 1;
        ft_memcpy(data->arena, champ->prog, champ->header->prog_size);
    }
    else
    {
    ...
    data->tab_proc[nb_proc - 1]->beg = 0;
    data->tab_proc[nb_proc - 1]->carry = 0;
    data->tab_proc[nb_proc - 1]->champ = champ->num;
    data->tab_proc[nb_proc - 1]->last_live = 0;
    data->tab_proc[nb_proc - 1]->pc = 0;
    data->tab_proc[nb_proc - 1]->registre[1] = champ->num;
}
