...
./asm_zaz $1_zaz.s
./asm $1.s
if [  ! -e $1CX.cor ] && [ ! -e $1CX.cor ]
then
	echo "$1 : [OK] NO FILE CREATE"
fi

if [ -e $1CX.cor ] && [ -e $1_zaz.cor ] && diff $1CX.cor $1_zaz.cor
then
	echo "$1: [OK]"	
	if [ -e $1CX.cor ]
		then
			hexdump -C $1CX.cor > ok_$1.txt
			rm $1CX.cor $1_zaz.cor
	fi
else
	 
	if [ -e $1CX.cor ] && [ -e $1_zaz.cor ] 
	then
		echo $1": [KO]"
		diff $1CX.cor $1_zaz.cor > diff_$1.txt
		echo "\n\n$1CX.cor \n" >> diff_$1.txt
		hexdump -C $1CX.cor >> diff_$1.txt
		echo "\n\n$1_zaz.cor\n" >> diff_$1.txt
